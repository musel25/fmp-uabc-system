...
export function tijuanaDate(now: Date): string {
  const parts = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Tijuana', year:'numeric', month:'2-digit', day:'2-digit' }).formatToParts(now)
...
export function earliestEventLocalDate(now: Date): string {
  ...
  let count=0
  while(count<MIN_LEAD_BUSINESS_DAYS) { cursor.setUTCDate(cursor.getUTCDate()+1); if(business(cursor)) count++ }
  return cursor.toISOString().slice(0,10)
}
export function meetsRegistrationLead(startLocal: string, now: Date): boolean {
  if (!/^\d{4}-\d{2}-\d{2}T(?:[01]\d|2[0-3]):[0-5]\d$/.test(startLocal)) return false
  return civil(startLocal.slice(0,10))!==null && startLocal.slice(0,10)>=earliestEventLocalDate(now)
}
export function latestRegistrationLocalDate(startLocalDate: string): string {
  ...
  let count=0
  while(count<MIN_LEAD_BUSINESS_DAYS) { if(business(cursor)) count++; cursor.setUTCDate(cursor.getUTCDate()-1) }
  return cursor.toISOString().slice(0,10)
}
