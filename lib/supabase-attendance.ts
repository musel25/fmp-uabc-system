...
import { dbRowToAttendanceSummary, dbRowToAttendanceResponse } from './event-mapper'
...
export async function getAttendanceSummary(eventId:string):Promise<AttendanceSummary>{
 const [settings,result]=await Promise.all([getWorkflowSettings(),supabase.from('attendance_sync_state').select('*').eq('event_id',eventId).maybeSingle()])
 if(result.error)throw result.error
 return dbRowToAttendanceSummary(eventId,result.data,settings.attendanceEnabled&&!!settings.attendancePrefillTemplate)
}
export async function getAttendancePage(eventId:string,cursor?:string):Promise<{responses:AttendanceResponse[];nextCursor:string|null}>{
 let query=supabase.from('attendance_responses').select('*').eq('event_id',eventId).order('source_response_id').limit(201)
