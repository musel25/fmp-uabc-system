...
import { filterReportRows,type ReportFilters,type ReportAnalyticsRow } from './report-analytics'
export async function getReportAnalyticsRows(filters:ReportFilters={}):Promise<ReportAnalyticsRow[]>{
 ...
 while(true){
  ...
  const events=data.map(dbRowToEvent),ids=events.map(e=>e.id)
  const [reports,progress]=await Promise.all([supabase.from('event_reports').select('*').in('event_id',ids),getEventProgress(ids)])
  if(reports.error)throw reports.error
  if(ids.some(id=>progress[id]?.state!=='loaded'))throw new Error('No se pudo consultar el seguimiento completo. Intenta de nuevo.')
  const byId=new Map((reports.data??[]).map(r=>[r.event_id,dbRowToEventReport(r)]))
  rows.push(...events.map(event=>({event,report:byId.get(event.id)??null,progress:progress[event.id]})))
